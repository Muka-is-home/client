...
const DropdownInputFields = ({ data, label, query, filterValues, setFilterValues, lg, sm, start, end, lastSm }) => {
  ...
  return (
    <>
      {data.slice(`${start && start}`, `${end && end}`).map((value, i) => (
        <Col lg={lg || value.size} sm={sm ? sm : lastSm ? i > 1  && lastSm : ''} key={value.name}>
